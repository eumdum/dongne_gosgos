...
#             if not is_valid:
#                 return Response({"error": f"유효하지 않은 사업자 번호입니다:{status_msg}"}, status=400)
...
def verify_biz(request):
    ...
    if is_valid:
        return Response({"message": f"확인 완료: {status_msg}"}, status=200)
    return Response({"error": f"가입 불가: {status_msg}"}, status=400)
...
def signup(request):
    ...
    username = data.get('username')
    password = data.get('password')
    b_no = data.get('business_number', '').replace('-', '').strip()
    ...
    try:
        user = User.objects.create_user(username=username, password=password, is_owner=(role == 'owner'))
        
        if role == 'owner':
            is_valid, _ = verify_business_number(b_no) # 내부에서 한 번 더 검증 (안전!)

            if not is_valid:
                user.delete()
                return Response({"error": f"유효하지 않은 사업자 번호입니다.{_}"}, status=400)
